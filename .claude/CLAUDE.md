...
Inventory groups: `local` (localhost), `ubuntu_test_vms`, `arch_test_vms`, and `dev_vms` (parent of both VM groups, sets `enable_picom: false` and the shared `ansible_user`). When prompted for `target_host`, enter one of these group names. Group vars live in `inventory/group_vars/<group>.yml`.
